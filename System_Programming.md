...
프로세스 사이의 데이터통신은 IPC를 통해서 이루어지고, "프로세스 간 통신"이라는 뜻

커널은 프로세스 통신을 위해 다양한 IPC 매커니즘을 제공

프로세스는 IPC를 통하여 다른 프로세스에게 데이터를송신하거나, 다른 프로세스로부터 데이터를 수신할 수 있음

EX) LCD에 출력할 문자 메세지를 전달, 센서로부터 읽은 값을 다른 프로세스에 전달, 디스플레이에 그릴 비트맵 데이터를 전달



#### 파이프(Pipe)

프로세스 사이에 형식없는 데이터의 교환, 파이프를 통해 전달되는 데이터는 단순한 바이트 스트림 형태

특징

* 파이프 안의 데이터는 바이트스트림(형식 없는 데이터)로 취급되기 때무에, 여러 메세지 객체가 쓰여졌을 떄, 메세지의 경계를 분간할 수 없음
* 파이프는 FIFO방식으로 데이터를 처리하므로, 들어온 순서대로 처리
* 고정된 크기(용량)
* 파이프의 데이터가 용량을 초과하면, 파이프에 데이터를 쓰는 프로세스는 "블록" 상태가 됨
* 반대로, 파이프가 텅 비어있으면, 파이프에서 데이터를 읽는 프로세스는 "블록"상태가 됨.



#### 메시지 큐(message queue)

* 파이프와 유사하게 프로세스들 간의 데이터 통신을 위한 IPC
* 경계 구분이 가능, 그러므로 하나의 메세지큐 안에 여러개의 메세지를 버퍼링 가능
* FIFO, 고정된 크기
* 많은 양의 데이터를 전달할 때에는 데이터 대신 그 데이터를 가리키고 있는 포인터만 전달하는 방법을 사용
* 양방향 통신이 가능
...
* 임의의 프로세스에서 특정 이벤트가 발생했다는 사실을 알려주는 매커니즘
* 커널 또는 프로세스에서 다른 프로세스에 어떤 이벤트가 발생되었는지를 알려주는 기법
* 각 프로세스에서 시그널 처리에 대해 동작 설정 가능
  * 시그널 무시
  * 시그널 블록
...
* 시스템 프로그램잉에서 꼭 익히는 내용 중 하나
* 쉘 스크립트 예

```
# 문제점
서버가 어느날 다운되었다. 확인해보니 서버 저장공간이 하나도 남지 않음
로그 파일 업데이트가 안되어 관련 프로그램 비정상종료 이유는 로그 파일이 많이 쌓여있음
